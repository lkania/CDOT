...
def objective(X, nu, signal, background_hat):
    ...
    scaled_signal_density = lambda_hat * signal(X=X, mu=mu_hat, sigma2=sigma2_hat).reshape(-1)
...
def psi(X, nu, signal, background_hat):
    grad_op = grad(
        fun=lambda nu, X, background_hat: objective(X=X, nu=nu, signal=signal, background_hat=background_hat),
        argnums=0,
        has_aux=False)
...
def psi_per_observation(X, nu, signal, background_hat):
    ...
    psi_ = vmap(lambda X, background_hat: psi(X=X, nu=nu, signal=signal, background_hat=background_hat))
...
def Psi(X, nu, signal, background_hat):
    nu = nu.reshape(-1)
    psis = psi_per_observation(X=X, nu=nu, signal=signal, background_hat=background_hat)
...
def influence(method, params):
    ...
    hessian_op = hessian(
        fun=lambda nu: objective(X=method.X, nu=nu, signal=params.signal.signal, background_hat=background_hat),
        argnums=0,
        has_aux=False)
